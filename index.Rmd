...
The National Mall is a unique place in Washington, D.C. and unlike any other census tract so let's exclude it from our analysis of violent crime and define outliers as the other tracts identified from our initial boxplot.
```{r}
...
However, the initial boxplot could use a little cleaning up. Below, we reproduce our boxplot, but using our new dataset excluding the National Mall and add labels identifying which tracts make up our group of outliers. We see that there are seven tracts that fall above the distribution given by the boxplot.
```{r}
...
Let's visualize this table by making a heatmap. We can see that among tracts with extreme violent crime rates, two tracts in particular account for a large share of homicides, while robberies, for example, are more equally distributed across these tracts.
```{r}
...
We take a quick look at what fields are available to us.
```{r}
names(DCtractMap)
...
Now we can produce our map. We start with a map of all census tracts in Washington, D.C. as our base map and add the layer of only census tract outliers on top, coloring them red so they stand out. Next, we add our water later for aesthetics, as well as a compass and scale bar.
```{r}
